    ...
    <x-slot name="header">
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('receipts.export.pdf', ['start_date' => $startDate, 'end_date' => $endDate]) }}" target="_blank" class="px-4 py-2 bg-rose-500/10 hover:bg-rose-500 hover:text-slate-800 border border-rose-500/20 text-rose-400 rounded-xl font-medium text-sm transition-all flex items-center gap-2 shadow-lg">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="glass-light rounded-2xl p-6">
                <form action="{{ route('reports.index') }}" method="GET" class="flex flex-col md:flex-row md:items-end gap-4">
                    <div class="flex-1">
                        <label class="block text-xs font-medium text-slate-500 mb-1">Periode Cepat</label>
                        <select name="period" onchange="this.form.submit()" class="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-slate-800 text-sm focus:border-primary-500 focus:ring-1 focus:outline-none">
                    ...
                    <div class="flex-1">
                        ...
                        <input type="date" name="start_date" value="{{ $startDate }}" class="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-slate-800 text-sm focus:border-primary-500 focus:ring-1 focus:outline-none [color-scheme:dark]">
                    </div>
                    <div class="flex-1">
                        <label class="block text-xs font-medium text-slate-500 mb-1">Sampai Tanggal</label>
                        <input type="date" name="end_date" value="{{ $endDate }}" class="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-slate-800 text-sm focus:border-primary-500 focus:ring-1 focus:outline-none [color-scheme:dark]">
                    </div>
                    <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div class="glass-light rounded-2xl p-6 relative overflow-hidden group hover:border-primary-500/30 transition-colors">
                    ...
                    <div class="relative z-10">
                        ...
                        <h3 class="text-2xl font-bold text-slate-800 font-display">Rp {{ number_format($totalAmount, 0, ',', '.') }}</h3>
                    </div>
                </div>

                <div class="glass-light rounded-2xl p-6 relative overflow-hidden group hover:border-emerald-500/30 transition-colors">
                    ...
                    <div class="relative z-10">
                        <div class="flex items-center gap-3 mb-3">
                            ...
                            <p class="text-xs font-medium text-slate-500 uppercase tracking-wider">Transaksi</p>
                        </div>
                        <h3 class="text-2xl font-bold text-slate-800 font-display">{{ $receipts->count() }} <span class="text-lg text-slate-500 font-normal">struk</span></h3>
                    </div>
                </div>

                <div class="glass-light rounded-2xl p-6 relative overflow-hidden group hover:border-cyan-500/30 transition-colors">
                    ...
                    <div class="relative z-10">
                        ...
                        <h3 class="text-2xl font-bold text-slate-800 font-display">Rp {{ number_format($avgPerTransaction, 0, ',', '.') }}</h3>
                    </div>
                </div>

                <div class="glass-light rounded-2xl p-6 relative overflow-hidden group hover:border-amber-500/30 transition-colors">
                    ...
                    <div class="relative z-10">
                        <div class="flex items-center gap-3 mb-3">
                            ...
                            <p class="text-xs font-medium text-slate-500 uppercase tracking-wider">vs Periode Lalu</p>
                        </div>
                        ...
                        @else
                            <h3 class="text-2xl font-bold text-slate-400 font-display">—</h3>
                        @endif
                    </div>
                </div>
            </div>
            ...
            <div class="glass-light rounded-2xl p-6">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-lg font-semibold text-slate-800">Tren Pengeluaran</h3>
                    <span class="text-xs text-slate-500">{{ $period === 'year' ? 'Per Bulan' : 'Per Hari' }}</span>
                </div>
                ...
                @else
                    <div class="flex flex-col items-center justify-center text-center p-8 h-[300px]">
                        <div class="w-14 h-14 rounded-2xl bg-slate-50 flex items-center justify-center mb-3">
                            <svg class="w-7 h-7 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 013 19.875v-6.75z"/></svg>
                        </div>
            ...
            <div class="glass-light rounded-2xl overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
                    <h3 class="text-sm font-semibold text-slate-600 uppercase tracking-wider">Detail Transaksi</h3>
                    <span class="text-xs text-slate-500">{{ $receipts->count() }} struk ditemukan</span>
                </div>
                            ...
                            <tbody class="divide-y divide-white/5 text-sm">
                                @foreach($receipts as $receipt)
                                    <tr class="hover:bg-slate-50 transition-colors">
                                        <td class="px-6 py-4 whitespace-nowrap text-slate-600">{{ $receipt->receipt_date->format('d M Y') }}</td>
                                        <td class="px-6 py-4 font-medium text-slate-800">{{ $receipt->store_name ?: 'Tidak diketahui' }}</td>
                                        <td class="px-6 py-4">
                                            <div class="text-xs text-slate-500 truncate max-w-xs" title="{{ $receipt->address }}">
                                                {{ $receipt->address ?: 'Tidak ada alamat' }}
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 font-medium text-primary-600 text-right">{{ number_format($receipt->total, 0, ',', '.') }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                ...
                @else
                    <div class="p-12 text-center">
                        ...
                        <h3 class="text-lg font-medium text-slate-800 mb-2">Tidak ada data</h3>
                        <p class="text-sm text-slate-500">Tidak ada pengeluaran pada periode tanggal yang dipilih.</p>
                    </div>
                @endif
            </div>
