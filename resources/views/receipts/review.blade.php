            ...
            <form action="{{ route('receipts.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div class="lg:col-span-4">
                        <div class="glass-light rounded-2xl p-4 sticky top-24">
                            <h3 class="text-sm font-semibold text-slate-500 mb-4 uppercase tracking-wider">Foto Struk</h3>
                            ...
                            <div class="mt-6 p-4 bg-primary-500/10 border border-primary-500/20 rounded-xl">
                                <h4 class="text-sm font-semibold text-primary-600 mb-1 flex items-center gap-2">
                                ...
                                <p class="text-xs text-slate-500 leading-relaxed">
                                    Silakan periksa kembali data di samping. Anda dapat mengedit nama barang, menyesuaikan kategori, atau memperbaiki harga jika ada kesalahan deteksi Gemini 2.5 Flash.
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="lg:col-span-8 space-y-6">
                        ...
                        <div class="glass-light rounded-2xl p-6">
                            <h3 class="text-sm font-semibold text-slate-500 mb-4 uppercase tracking-wider">Informasi Umum</h3>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                <div>
                                    <label class="block text-sm font-medium text-slate-600 mb-1.5">Nama Toko</label>
                                    <input type="text" name="store_name" value="{{ old('store_name', $aiData['store_name']) }}" required
                                        class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 focus:border-primary-500 focus:ring-1 focus:outline-none text-sm">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-slate-600 mb-1.5">Tanggal Struk</label>
                                    @php
                                    ...
                                    <input type="date" name="receipt_date" value="{{ old('receipt_date', $parsedDate) }}" required
                                        class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 focus:border-primary-500 focus:ring-1 focus:outline-none text-sm [color-scheme:dark]">
                                </div>
                                <div class="sm:col-span-2">
                                    <label class="block text-sm font-medium text-slate-600 mb-1.5">Total Pengeluaran (Rp)</label>
                                    <input type="number" name="total" value="{{ old('total', $aiData['total']) }}" required min="0" step="1" x-model="total" readonly
                                        class="w-full px-4 py-2.5 bg-slate-100 border border-slate-200 rounded-xl text-primary-600 font-bold text-lg focus:outline-none">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="glass-light rounded-2xl p-6">
                            <div class="flex items-center justify-between mb-6">
                                <h3 class="text-sm font-semibold text-slate-500 uppercase tracking-wider">Daftar Barang yang Terdeteksi</h3>
                                <button type="button" @click="addItem" class="text-sm font-medium text-primary-600 hover:text-primary-300 flex items-center gap-1">
                            ...
                            <div class="space-y-4">
                                <template x-for="(item, index) in items" :key="item.key">
                                    <div class="p-4 rounded-xl bg-slate-50 border border-slate-200 relative hover:border-white/20 transition-colors">
                                        <button type="button" @click="removeItem(index)" class="absolute top-3 right-3 text-slate-400 hover:text-rose-400 p-1">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
                                        </button>

                                        <div class="grid grid-cols-1 sm:grid-cols-12 gap-4">
                                            <div class="sm:col-span-5">
                                                <label class="block text-xs font-medium text-slate-500 mb-1">Nama Barang</label>
                                                <input type="text" x-model="item.name" :name="'items['+index+'][item_name]'" required
                                                    class="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-slate-800 text-sm focus:border-primary-500 focus:ring-1 focus:outline-none">
                                            </div>
                                            
                                            <div class="sm:col-span-4">
                                                <label class="block text-xs font-medium text-slate-500 mb-1">Kategori (Auto)</label>
